...
- v1.4: 数学的解析セクション追加
- v1.5: PPTX出力対応、游ゴシックに統一
- v1.6: 埋め込み手法の限界（文脈非依存性）について追記
...
本セクションでは、現在の実装が抱える本質的な限界と、その改善方向を整理します。
ユーザーが結果を解釈する際、また将来的に拡張を検討する際の参考としてください。

### 9.1 埋め込みの文脈非依存性

本実装では、抽出された各単語について `text-embedding-3-small` に**単語文字列のみ**を渡して埋め込みを取得しています。文書中での出現文脈は API に渡していません。

```python
# semantic_wordcloud.py 内の get_embeddings()
response = client.embeddings.create(
    model="text-embedding-3-small",
    input=batch  # ["機械学習", "深層学習", ...] のような単語文字列リスト
)
```

#### 帰結

1. **ユーザーコーパス固有の意味は反映されない**
   - 得られるベクトルは、OpenAI の学習コーパスにおける各単語の「デフォルト的な」意味表現
   - 特定の分野・組織・文書群における独特な使われ方は反映されない
   - 例: 「設計」という語は、本実装では工学的な文脈に近いベクトルを返す可能性が高いが、もし入力コーパスが社会学系で「制度設計」のような用法が中心であっても、その文脈は反映されない

2. **多義語は単一ベクトルに潰れる**
   - 同じ表記の単語が異なる意味で使われていても、配置上は 1 点として扱われる
   - 例: 「モデル」が機械学習文脈と数理モデル文脈の両方で出現する場合、両者を区別せず、単一のベクトル（OpenAI モデル内部での「モデル」という語の汎用的表現）が割り当てられる
   - 文脈ごとの出現ベクトルを平均化しているわけですらない（そもそも文脈を見ていない）

### 9.2 改善方向

| 方向 | 手法 | トレードオフ |
|------|------|------|
| 文脈付き埋め込み（語ごと平均） | 単語の各出現箇所について周辺文を含めて API に投げ、得られたベクトルを語ごとに平均化 | コーパス固有性は反映されるが、多義性は依然潰れる。API 呼び出し回数・コストが大幅に増加 |
| 出現箇所のクラスタリング | 各出現を個別に文脈付きで埋め込み、クラスタリングにより多義語を「モデル(ML)」「モデル(数理)」のように分割表示 | 多義性は捉えられる。ただしワードクラウドとしての見た目が複雑化し、解釈の負荷も増える |

### 9.3 設計上の判断

現在の実装は、

- **計算コスト**: 単語単位で 1 回の API 呼び出し（語数 N に比例）
- **キャッシュ効率**: 単語をキーとした再利用が容易（`embeddings_cache.json`）
- **再現性**: 同じ語に対しては常に同じベクトル

を優先した設計です。研究領域の俯瞰的な可視化という用途では実用上十分な結果が得られますが、上記の限界を理解した上で使用することが重要です。特に、出力結果を「特定コーパスの意味構造の忠実な再現」と解釈するのではなく、「汎用的な意味空間における語の配置」として解釈する方が、誤った推論を避けられます。
